...
function drawDonutChart(containerId, data, centerText) {
  ...
  const cy = size / 2;
  const radius = 55;
  const labelRadius = 75; // Khoảng cách chữ bay ra
  let currentAngle = 0;
  ...
  data.forEach((item) => {
    ...
    // Tọa độ đường kẻ chỉ dẫn (Annotation)
    ...
    const ly2 = cy + labelRadius * Math.sin(midAngle);

    svgHtml += `
        <g class="chart-group">
            <path d="${pathData}" fill="${item.color}" class="chart-segment" 
                  onmousemove="showTooltip(evt, '${item.label}: ${item.info}')" 
                  onmouseleave="hideTooltip()">
            </path>
            
            <line x1="${lx1}" y1="${ly1}" x2="${lx2}" y2="${ly2}" class="chart-line" />
            <circle cx="${lx2}" cy="${ly2}" r="2" fill="${
      item.color
    }" class="chart-line" />
            
            <rect x="${lx2 - 30}" y="${
      ly2 - 15
    }" width="60" height="30" rx="4" class="chart-text-bg" />
            
            <text x="${lx2}" y="${ly2 - 4}" class="chart-text">${
      item.label
    }</text>
            <text x="${lx2}" y="${
      ly2 + 10
  ...
  // Lỗ tròn trắng ở giữa (Tạo hình Donut)
  svgHtml += `<circle cx="${cx}" cy="${cy}" r="${
    radius * 0.6
  }" fill="white" />`;
  svgHtml += `</svg><div class="center-label">${centerText}</div>`;
...
// --- 3. CÁC HÀM LOGIC KHÁC (GIỮ NGUYÊN) ---
...
function switchView(viewId) {
  ...
  // Fix lỗi null nếu click vào menu con
...
function renderStaff(data = staffData) {
  ...
  container.innerHTML = "";
  data.forEach((staff) => {
    const stCls = staff.status === "online" ? "dot-online" : "dot-offline";
    const stTxt = staff.status === "online" ? "Đang làm việc" : "Offline";
    container.innerHTML += `<div class="staff-card"><img src="${staff.img}" class="staff-avatar"><div class="staff-info"><div class="staff-name">${staff.name}</div><div class="staff-role">${staff.role}</div><div style="font-size:0.8rem;margin-top:8px;"><span class="staff-status-dot ${stCls}"></span> ${stTxt}</div></div><div class="staff-actions"><button class="icon-btn btn-edit" onclick="openEditStaff(${staff.id})"><i class="fas fa-pen"></i></button><button class="icon-btn btn-delete" onclick="deleteStaff(${staff.id})"><i class="fas fa-trash"></i></button></div></div>`;
  });
}
...
function saveStaff() {
  ...
  if (isEditingStaff) {
    ...
    if (idx !== -1) {
      staffData[idx] = { id: currentStaffId, name, role, status, img };
      addNotification(`Cập nhật nhân viên: ${name}`, "info");
    }
  } else {
    staffData.push({ id: Date.now(), name, role, status, img });
